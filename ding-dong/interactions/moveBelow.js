...
const moveBelow = async (interaction, client) => {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith("move-modal-below-")) {
        ...
        try {
            ...
            } else if (destChannel.isThread()) {
                if (!threadId) {
                    await interaction.editReply("❌ Title is required for threads!");
                    return;
                }
            ...
            const mesageToMove = [targetMessage];

            if (thread) {
                await interaction.editReply(`I'm going to move ${mesageToMove.length} messages from ${sourceChannel.name} to ${destChannel.name} in thread ${thread.name}`);
            } else {
                await interaction.editReply(`I'm going to move ${mesageToMove.length} messages from ${sourceChannel.name} to ${destChannel.name}`);
            }

        } catch (error) {
            console.log(error);
            await interaction.editReply("An error occurred while moving messages");
        }
    }
