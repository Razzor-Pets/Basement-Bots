const pokemonSpawn = async (message) => {
    if (message.embeds.length > 0 && message.embeds[0].title) {
        if (message.embeds[0].title.includes("A wild pokémon has appeared!") || message.embeds[0].title.includes("A new wild pokémon has appeared!") && message.channel.id == process.env.POKEMON_SPAWN_CHANNEL) {
